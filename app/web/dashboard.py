...
# حماية بسيطة بكلمة مرور من الإعدادات (يمكن تحسينها لاحقاً)
def admin_guard(request: Request):
    token = request.cookies.get("admin_auth")
    if token == settings.ADMIN_PASSWORD:
        return True
    raise_html = """
    <form method="post" action="/dashboard/login" style="max-width:320px;margin:80px auto;font-family:sans-serif">
      <h3>تسجيل دخول</h3>
      <input name="password" type="password" placeholder="كلمة المرور" style="width:100%;padding:10px;margin:10px 0">
      <button type="submit" style="padding:10px 14px">دخول</button>
    </form>"""
    return HTMLResponse(raise_html, status_code=401)

@router.post("/dashboard/login")
def dashboard_login(request: Request):
    # بسيط: يستخرج الباسوورد من body
    async def _inner():
        form = await request.form()
        pwd = form.get("password", "")
        if pwd == settings.ADMIN_PASSWORD:
            resp = HTMLResponse('<script>location.href="/dashboard";</script>')
            resp.set_cookie("admin_auth", pwd, httponly=True, max_age=7*24*3600)
            return resp
        return HTMLResponse("Unauthorized", status_code=401)
    return _inner()

@router.get("/dashboard", response_class=HTMLResponse)
def dashboard(_: Any = Depends(admin_guard)):
    ...
    try:
        ...
        data: Dict[str, Any] = {
            "provider_balance": _dec(provider_balance),
        ...
        # نفس الـ HTML/CSS تبعك
        html = f"""<!doctype html><html dir="rtl" lang="ar"><head>
<meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<style>
:root{{--bg:#0f172a;--card:#111827;--muted:#94a3b8;--text:#e5e7eb;--accent:#22d3ee;--accent-2:#60a5fa;--border:#1f2937;--table-stripe:rgba(255,255,255,0.03);--shadow:0 10px 25px rgba(0,0,0,0.25);--radius:14px}}
@media (prefers-color-scheme: light){{:root{{--bg:#f7f7fb;--card:#fff;--muted:#6b7280;--text:#0f172a;--accent:#0ea5e9;--accent-2:#2563eb;--border:#e5e7eb;--table-stripe:rgba(0,0,0,0.02)}}}}
body{{margin:0;padding:24px;font-family:-apple-system,Segoe UI,Roboto,"Cairo",Tahoma,Arial,sans-serif;background:radial-gradient(1000px 600px at 100% -10%, rgba(34,211,238,0.08), transparent 60%),radial-gradient(800px 500px at -10% 0%, rgba(96,165,250,0.08), transparent 60%),var(--bg);color:var(--text)}}
.header{{background:linear-gradient(90deg, rgba(34,211,238,0.20), rgba(96,165,250,0.20));border:1px solid var(--border);border-radius:var(--radius);padding:20px 18px;box-shadow:var(--shadow);display:flex;flex-wrap:wrap;justify-content:space-between;gap:12px;align-items:center}}
.header h2{{margin:0;font-size:22px}}.meta{{color:var(--muted);font-size:13px}}.pill{{display:inline-block;padding:2px 8px;border-radius:999px;font-size:12px;border:1px solid var(--border);background:rgba(34,211,238,0.10);color:var(--accent-2)}}
...
.card::after{{content:"";position:absolute;inset:-20% -10% auto auto;width:160px;height:160px;transform:rotate(35deg);background:linear-gradient(180deg, transparent, rgba(34,211,238,0.10));border-radius:24px;pointer-events:none}}
.card h4{{margin:0 0 8px 0;font-size:15px;color:var(--muted)}}.card .value{{font-size:28px;font-weight:700;letter-spacing:0.2px}}.card .sub{{margin-top:6px;color:var(--muted);font-size:13px}}
.grid-2{{display:grid;grid-template-columns:1fr 1fr;gap:16px}}
@media (max-width:1100px){{.grid-2{{grid-template-columns:1fr}}}}
table{{width:100%;border-collapse:separate;border-spacing:0;overflow:hidden;border:1px solid var(--border);border-radius:var(--radius);background:var(--card);box-shadow:var(--shadow)}}
thead th{{text-align:right;padding:12px;font-size:13px;color:var(--muted);border-bottom:1px solid var(--border);background:linear-gradient(180deg, rgba(255,255,255,0.02), transparent)}}
...
<section class="grid-cards">
  <div class="card"><h4>رصيدي عند المزوّد</h4><div class="value">{_fmt_money(data['provider_balance'])} USD</div><div class="sub">الحساب: {data['provider_email'] or 'غير متاح'} • المصدر: /profile</div></div>
  <div class="card"><h4>إجمالي تعبئة الرصيد</h4><div class="value">{_fmt_money(data['total_topups'])} USD</div><div class="sub">اليوم: {_fmt_money(data['topups_today'])} • هذا الشهر: {_fmt_money(data['topups_month'])}</div></div>
  <div class="card"><h4>مبيعات الشحن</h4><div class="value">{_fmt_money(data['orders_total_usd'])} USD</div><div class="sub">عدد الطلبات: {_fmt_int(data['orders_count'])} • اليوم: {_fmt_money(data['orders_today_usd'])} • هذا الشهر: {_fmt_money(data['orders_month_usd'])}</div></div>
  <div class="card"><h4>الربح</h4><div class="value">{_fmt_money(data['profit_total'])} USD</div><div class="sub">اليوم: {_fmt_money(data['profit_today'])} • هذا الشهر: {_fmt_money(data['profit_month'])}</div></div>
</section>

<section class="grid-2">
  <div class="card" style="padding:0;">
    <table>
      <thead><tr><th>طريقة الشحن</th><th>المجموع (USD)</th></tr></thead>
      <tbody>{"".join([f"<tr><td>{t}</td><td>{_fmt_money(v)}</td></tr>" for t, v in data['topups_by_type']])}</tbody>
    </table>
  </div>
  <div class="card" style="padding:0;">
    <table>
      <thead><tr><th>المنتج</th><th>طلبات</th><th>كمية</th><th>المجموع (USD)</th></tr></thead>
      <tbody>{
        "".join([f"<tr><td>{name}</td><td>{_fmt_int(orders)}</td><td>{_fmt_int(qty)}</td><td>{_fmt_money(usd)}</td></tr>"
