...
:root {{
  ...
  --accent:#22d3ee; --accent2:#60a5fa; --border:#1f2937; --danger:#ef4444; --ok:#22c55e;
  --shadow:0 10px 25px rgba(0,0,0,0.25); --radius:14px;
}}
@media (prefers-color-scheme: light) {{
  :root {{
    --bg:#f7f7fb; --card:#fff; --muted:#6b7280; --text:#0f172a;
    --accent:#0ea5e9; --accent2:#2563eb; --border:#e5e7eb; --danger:#dc2626; --ok:#16a34a;
  }}
}}
* {{ box-sizing:border-box }}
body {{
  margin:0; padding:22px; font-family:-apple-system, Segoe UI, Roboto, "Cairo", Tahoma, Arial, sans-serif;
  background: radial-gradient(1000px 600px at 100% -10%, rgba(34,211,238,.08), transparent 60%),
              radial-gradient(800px 500px at -10% 0%, rgba(96,165,250,.08), transparent 60%), var(--bg);
  color:var(--text);
}}
.header {{
  background:linear-gradient(90deg, rgba(34,211,238,.20), rgba(96,165,250,.20));
...
.btn {{
  ...
  background:var(--card); color:var(--text); text-decoration:none; cursor:pointer; font-size:14px;
}}
.btn.primary {{ background:linear-gradient(180deg, var(--accent), var(--accent2)); color:#fff; border:none }}
.btn.danger {{ border-color:var(--danger); color:var(--danger) }}
.btn.ok {{ border-color:var(--ok); color:var(--ok) }}
.input, .select {{
  padding:8px 10px; border:1px solid var(--border); border-radius:10px; background:var(--card); color:var(--text); min-width:160px;
}}
.table-wrap {{
  margin-top:16px; background:var(--card); border:1px solid var(--border); border-radius:var(--radius); overflow:auto; box-shadow:var(--shadow);
}}
table {{ width:100%; border-collapse:separate; border-spacing:0 }}
thead th {{
  text-align:right; padding:12px; font-size:13px; color:var(--muted); border-bottom:1px solid var(--border);
...
.badge.block {{ background:rgba(239,68,68,.12); color:var(--danger) }}
...
.form-card {{
  margin-top:16px; background:var(--card); border:1px solid var(--border); border-radius:var(--radius);
...
class UsersView(BaseView):
    ...
    @expose("admin/users", methods=["GET"])
    def list_users(self, request: Request):
        ...
        try:
            ...
            if q:
                ...
                base = base.filter(
                    or_(
                        func.cast(User.id, str).ilike(like),
                        func.cast(User.tg_id, str).ilike(like),
            ...
            tr = "".join(
                ...
                <tr>
                  ...
                  <td>
                    <a class="btn" href="admin/users/{r.id}/edit">تعديل</a>
            ...
            if pages > 1:
                ...
                for i in range(1, pages + 1):
                    ...
                    items.append(
                        f'<a class="{cls}" href="admin/users?'
...
<section class="header">
  ...
  <div class="actions">
    <a class="btn primary" href="admin/users/new">+ مستخدم جديد</a>
  </div>
</section>
...
"""
    ...
    # ------- Create -------
    @expose("admin/users/new", methods=["GET", "POST"])
    async def create_user(self, request: Request):
        if request.method == "POST":
            ...
            try:
                ...
                existing_wallet = db.query(Wallet).filter(Wallet.user_id == user.id, Wallet.currency == "USD").one_or_none()
    ...
    # ------- Edit -------
    @expose("admin/users/{user_id}/edit", methods=["GET", "POST"])
    async def edit_user(self, request: Request, user_id: int):
        ...
        try:
            ...
            if request.method == "POST":
                ...
                if not tg_id.isdigit():
                    body = _form_user(name, tg_id, country, is_blocked, balance, error="Telegram ID يجب أن يكون رقمًا", submit_text="حفظ التعديلات", back_href="admin/users")
    ...
    # ------- Delete -------
    @expose("admin/users/{user_id}/delete", methods=["POST"])
